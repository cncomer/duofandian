			...
			for (NetworkInfo i : s) {
				if (i.getTypeName().contentEquals("WIFI")) {
					...
					if (state == NetworkInfo.State.CONNECTED && ssid != null) {
						...
						Toast.makeText(context, parent.getString(R.string.wifi_connected_to) + ssid, Toast.LENGTH_LONG).show();
						// Runnable delayKill = new Killer(parent);
						// delayKill.run();
					}
					if (state == NetworkInfo.State.DISCONNECTED && ( ssid != null && ssid.equals(parent.getCurrentSSID())) ) {
